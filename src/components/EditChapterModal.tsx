...
export default function EditChapterModal({ subjectId, chapterId, currentName, onClose }: EditChapterModalProps) {
    const { editChapter } = useStudy();
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (name.trim()) {
            editChapter(subjectId, chapterId, name.trim());
            onClose();
        }
    };
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                ...
                >
                    <div className="p-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/50">
                        <h2 className="text-lg font-bold text-slate-800 dark:text-white">Edit Chapter</h2>
                        <button
                            onClick={onClose}
                            aria-label="Close"
