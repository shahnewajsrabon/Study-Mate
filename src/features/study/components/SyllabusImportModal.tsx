...
import { SYLLABUS_TEMPLATES, type SyllabusTemplate, type TemplateSubject, type TemplateChapter } from '../data/syllabusTemplates.ts';
...
export default function SyllabusImportModal({ onClose }: SyllabusImportModalProps) {
    ...
    const currentTemplate = SYLLABUS_TEMPLATES.find((t: SyllabusTemplate) => t.id === selectedTemplateId);
    ...
    const handleSelectAll = () => {
        ...
        if (selectedSubjects.size === currentTemplate.subjects.length) {
            ...
            const allNames = new Set(currentTemplate.subjects.map((s: TemplateSubject) => s.name));
                ...
                <div className="flex-1 overflow-hidden flex flex-col md:flex-row">
                    ...
                    <div className="w-full md:w-48 bg-slate-50 dark:bg-slate-900/50 border-r border-slate-100 dark:border-slate-700 p-4">
                        ...
                        <div className="space-y-2">
                            {SYLLABUS_TEMPLATES.map((t: SyllabusTemplate) => (
                                <button
                                    key={t.id}
                                    onClick={() => {
                    ...
                    <div className="flex-1 p-6 overflow-y-auto">
                        ...
                        <div className="space-y-3">
                            {currentTemplate?.subjects.map((subject: TemplateSubject) => (
                                <div
                                    key={subject.name}
                                ...
                                >
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-1">
                                            {subject.chapters.slice(0, 3).map((c: TemplateChapter) => c.name).join(', ')}...
                                        </p>
                                    </div>
                                </div>
                            ))}
