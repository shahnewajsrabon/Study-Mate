...
import type { Chapter, Topic } from '../types/study.ts';
                ...
                <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 mt-1">
                        <span className="text-xs text-slate-400 dark:text-slate-500 font-medium whitespace-nowrap transition-colors">
                            {chapter.topics?.filter((t: Topic) => t.isCompleted).length || 0}/{chapter.topics?.length || 0} Topics
                        ...
                        {chapter.topics && chapter.topics.length > 0 && (
                            <div className="h-1.5 w-24 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden flex-shrink-0 transition-colors">
                                <motion.div
                                    initial={{ width: 0 }}
                                    animate={{ width: `${Math.round((chapter.topics.filter((t: Topic) => t.isCompleted).length / chapter.topics.length) * 100)}%` }}
